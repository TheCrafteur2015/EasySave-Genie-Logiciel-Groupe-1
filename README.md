...
**EasySave** est une application console robuste conçue pour automatiser et sécuriser vos travaux de sauvegarde. Développée en **C# (.NET 8.0)**, elle utilise une architecture modulaire pour offrir une solution flexible, multilingue et performante.
...
* **Interface Multilingue :** Support complet du **Français** et de l'**Anglais**, avec chargement dynamique des ressources JSON.
...
* **Système de Logs Flexible :** Génération de journaux quotidiens (format Texte par défaut, extensible en JSON/XML) incluant les temps de transfert et de cryptage.
* **Mode Commande :** Exécution via menu interactif ou arguments de ligne de commande (ID unique, liste `;` ou intervalle `-`).
